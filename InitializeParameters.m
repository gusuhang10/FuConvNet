function cnn = InitializeParameters(cnn,opts)
    ...
    for l = 1:numel(cnn.layers)
        ...
        if(strcmp(layer.type,'c'))%convolutional layer
           ...
           numfuzzypartition_mat = [5 5 5 5];
           [~,~,num_channel,~] = size(layer.W);
           numfuzzypartiton_con_mat = zeros(1,num_channel);
           for i = 1:num_channel
               numfuzzypartiton_con_mat(i) = numfuzzypartition_mat(randi(length(numfuzzypartition_mat)));
           end
           layer.numfuzzypartition_conv = numfuzzypartiton_con_mat;     
